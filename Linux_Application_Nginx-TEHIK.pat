      ...
      public class PatternExec {        
        public void PerformAction(object[] parameters)
        {
            try
            {
                ...
                // Find Nginx service
                ApplicationServiceInfo nginxService = host.ApplicationServices.Find(o => o.Name.Contains("nginx"));
        ...
        private static void GetServerInfo(string input, ref string sname, ref string sport, ref string sroot, ref string ssl)
        {
          ...
          for (int x = 0; x < lines.Length; x++) {
            ...
            if (lines[x].Contains("listen")) {
              ...
              if (sport.Contains(":")) {
                int i1 = sport.LastIndexOf(":") + 1;
                i2 = sport.IndexOf(" ");

                if (i2 == -1) {
                  i2 = sport.LastIndexOf(";");
                }

                sport = sport.Substring(i1, i2 - i1).Trim();

                sname = sport.Substring(0, i1).Trim();
                if (sname.Contains(":")) {
                  sname = "_";
                }
              }
              ...
              i2 = sname.IndexOf(" ");
              if (i2 == -1) {
                i2 = sname.LastIndexOf(";");
              }

              sname = sname.Substring(0, i2);
            }
          }
        }
