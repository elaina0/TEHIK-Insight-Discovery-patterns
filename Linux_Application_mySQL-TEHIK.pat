                ...
                if (!host.ApplicationServices.IsNullOrEmpty())
                {
                    ...
                    if (!string.IsNullOrEmpty(commandResult) && commandResult.Contains("version")) {
                        ...
                        if (Service != null) {
                            ...
                            List<ApplicationInfo> mysqllist = null;
                            if (!host.Applications.IsNullOrEmpty()) {
                                if (Service.Name.Contains("maria")) {
                                    mysqllist = host.Applications.FindAll(o => o.Name.ToLower().Contains("mariadb") && o.Version == InstanceInfo.version && o.Description.ToLower().Contains("server"));
                                }
                                else {
                                    mysqllist = host.Applications.FindAll(o => o.Name.ToLower().Contains("mysql") && o.Version == InstanceInfo.version && o.Description.ToLower().Contains("server"));
                                }
                            }

                            foreach (ApplicationInfo m in mysqllist) {
                                if (m.ReferencedDatabases == null)
                                    m.ReferencedDatabases = new List<string>();

                                m.ReferencedDatabases.Add(dbi.ObjectHash);
                            }
